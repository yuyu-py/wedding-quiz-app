...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function showScreen(screen) {
    ...
    console.log(`Display画面遷移: ${screen.id}に移動しました`);
  }
  ...
  async function markAnswerAsDisplayed(quizId) {
    try {
      ...
      // エラーの場合は再試行
      setTimeout(() => markAnswerAsDisplayed(quizId), 1000);
    }
  }
  ...
  async function showQuestion(quizId) {
    try {
      ...
      // エラーが発生した場合は再試行
      setTimeout(() => showQuestion(quizId), 2000);
    }
  }
  ...
  async function showAnswer(quizId) {
    try {
      ...
      // エラーが発生した場合は再試行
      setTimeout(() => showAnswer(quizId), 2000);
    }
  }
  ...
  async function showRanking(position = 'all') {
    try {
      ...
      // エラーの場合は再試行
      setTimeout(() => showRanking(position), 2000);
    }
  }
  ...
  socket.on('timer_sync', (data) => {
    ...
    if (currentQuizId === quizId && currentScreen === quizQuestionScreen) {
      ...
      if (timeLeft > 0) {
        startTimer(timeLeft);
      } else if (timeLeft <= 0 && !floatingTimer.classList.contains('hidden')) {
        // タイマーが0で、まだタイマーが表示されている場合は時間切れ処理
        stopTimer();
        // タイマー終了イベントをサーバーに送信
        socket.emit('timer_expired', { quizId: currentQuizId });
        // 解答画面に自動遷移
  ...
  socket.on('quiz_event', (data) => {
    ...
    // 再送イベントの場合、現在の状態と矛盾しないか確認
    if (isRetry) {
      console.log('再送イベント受信:', event, quizId);
      
      // 例えば show_answer イベントの再送なのに、すでに答え画面を表示している場合は無視
      if (event === 'show_answer' && currentScreen === quizAnswerScreen) {
        console.log('既に答え画面を表示しているため、再送イベントを無視します');
        return;
      }
    }
