// public/js/player.js - タイマー処理の完全修正
...
let timerStartTime = 0;   // タイマー開始時刻（サーバー時刻）
let timerEndTime = 0;     // タイマー終了予定時刻（サーバー時刻）
...
// タイマーを開始する関数 - サーバー時刻同期対応に修正
function startTimer(seconds = 30) {
  clearInterval(timerInterval);
  timeLeft = seconds;
  timerValue.textContent = timeLeft;
  
  console.log(`Player: タイマー開始 ${seconds}秒 - 開始時刻: ${timerStartTime}, 終了時刻: ${timerEndTime}`);
  
  timerInterval = setInterval(() => {
    // 現在の推定サーバー時刻に基づいて残り時間を計算
    const now = Date.now() + serverTimeOffset;
    const msRemaining = timerEndTime - now;
...
async function fetchAndShowQuestion(quizId) {
  ...
  try {
    ...
    // タイマーは開始しない（サーバーからのタイマー開始イベントを待つ）
    console.log(`Player: 問題画面表示完了 (ID: ${quizId}) - タイマーはサーバーからの指示待ち`);
...
function initSocketConnection() {
  ...
  socket.on('timer_start', (data) => {
    const { quizId, startTime, duration, remainingTime } = data;
    
    console.log(`Player: タイマー開始イベント受信: クイズ ${quizId} - ${duration}秒 (残り${remainingTime}秒)`);
    
    if (quizId === currentQuizId && currentScreen === quizScreen) {
      // 既存のタイマーがあれば停止
      clearInterval(timerInterval);
      ...
      const estimatedLatency = 100; // 推定値
      serverTimeOffset = (startTime + ((duration - remainingTime) * 1000)) - receivedTime;
      
      console.log(`Player: サーバー時刻オフセット設定: ${serverTimeOffset}ms`);
      
      // 表示を更新
      timerValue.textContent = timeLeft;
      console.log(`Player: タイマー表示を${timeLeft}秒に設定`);
      ...
      console.log(`Player: タイマー開始条件不一致 - 現在のクイズID: ${currentQuizId}, 画面: ${currentScreen.id}`);
    }
  });
  ...
  socket.on('timer_sync', (data) => {
    ...
    if (currentQuizId === quizId && currentScreen === quizScreen) {
      // 時刻差を更新
      const receivedTime = Date.now();
      const estimatedLatency = 100; // 片道レイテンシの推定値
      serverTimeOffset = timestamp - receivedTime + estimatedLatency;
      
      // デバッグログ（頻度を減らす）
      if (remainingTime % 5 === 0) {
        console.log(`タイマー同期: ${remainingTime}秒、オフセット: ${serverTimeOffset}ms`);
      }
      
      // 既存のタイマーが動いていれば何もしない（timer_startイベントで開始済み）
      if (!timerInterval) {
        // 念のためタイマーが動いていなければ再開
        console.log('タイマーが動いていないので再開します');
  ...
  socket.on('quiz_event', (data) => {
    const { event, quizId, position, auto, manual, isQuestionStart } = data;
    console.log('イベント受信:', event, quizId, position, 
               (auto ? '自動遷移' : '') + (manual ? '手動遷移' : '') + (isQuestionStart ? '問題開始' : ''));
    ...
    switch (event) {
      ...
      case 'next_slide':
        ...
        if (displayCurrentScreen === 'quiz_title') {
          // タイトル画面から問題表示へ
          console.log('Player: タイトル画面から問題画面へ遷移');
  ...
  socket.on('disconnect', () => {
    console.log('Socket.io から切断されました');
    // 必要に応じて再接続処理を追加
  });
}
